...
class Account:
    ...
    def zakup(self, identyfikator, cena_sztuki, liczba_sztuk):
        if cena_sztuki > 0 < liczba_sztuk:
            if (self.saldo_kwota - (cena_sztuki * liczba_sztuk)) > 0:
                self.dzialanie_magazyn(identyfikator, liczba_sztuk)
                self.saldo_kwota -= (cena_sztuki * liczba_sztuk)
                self.zapis_akcji("zakup", [identyfikator, cena_sztuki, liczba_sztuk])
                return True
            else:
                print('Brak wystarczających środków na koncie')
                return False
    ...
    def sprzedaz(self, identyfikator, cena_sztuki, liczba_sztuk):
        if cena_sztuki > 0 < liczba_sztuk:
            if self.dzialanie_magazyn(identyfikator, liczba_sztuk * (-1)):
                self.saldo_kwota += (cena_sztuki * liczba_sztuk)
                self.zapis_akcji("sprzedaż", [identyfikator, cena_sztuki, liczba_sztuk])
                return True
            else:
                return False
        else:
